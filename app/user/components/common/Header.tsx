        ...
        <div className="d-md-none border-top bg-white">
          <div className="p-3 d-flex flex-column gap-2">
            ...
            {[...corporateItems, ...providerItems, ...securityItems].map((item) => (
              item.href.startsWith('/') ? (
                <Link key={item.label} href={item.href} className="text-decoration-none text-secondary fw-medium py-2 px-2">
                  {item.label}
                </Link>
              ) : (
                <a key={item.label} href={item.href} className="text-decoration-none text-secondary fw-medium py-2 px-2">
                  {item.label}
                </a>
              )
            ))}
