...
export default async function EditLabTestOrderPage({
  params,
}: {
  ...
  const types = await prisma.lab_labtesttype.findMany({
  ...
  const selectOptions = {
    LabTestTypeID: types.map((t) => ({
      label: t.LabTestTypeName,
      value: t.LabTestTypeID,
    })),
