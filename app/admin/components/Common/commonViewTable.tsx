...
export function ViewTable({ columns, data }: DetailViewTableProps) {
  ...
  const auditKeys = ['Created', 'Modified', 'CreatedByUserID', 'ModifiedByUserID', 'CreatedAt', 'UpdatedAt', 'IsDeleted'];
  // Keys usually used for headers/titles
  const titleKeys = ['Name', 'Title', 'Subject', 'PatientName', 'DoctorName', 'CityName'];
  ...
  const titleField = columns.find(col => col.accessor && titleKeys.includes(col.accessor));
  const cardTitle = titleField && data[titleField.accessor!] ? data[titleField.accessor!] : 'Record Details';

  // Helper to formatting values
  const formatValue = (rawValue: any) => {
    if (rawValue === null || rawValue === undefined) return <span className="text-gray-400 italic">N/A</span>;
          ...
          <Box sx={{ mb: 4 }}>
            ...
            <Grid container spacing={4}>
              {primaryInfo.map((col, index) => {
                ...
                return (
                  <Grid item xs={12} md={isLongText ? 12 : 6} key={index}>
                    <Box sx={{
                      p: 2,
                      borderRadius: 2,
                      ...
                      <Typography variant="body1" sx={{ mt: 1, wordBreak: 'break-word', color: 'text.primary' }}>
