...
export default async function AddMedicineOrderPaymentTypePage() {
    ...
    const medicineOrderPaymentTypes = await prisma.phm_medicineorderpaymenttype.findMany({ where: { IsDeleted: false } });

    const selectOptions = {
        ParentPaymentTypeID: medicineOrderPaymentTypes.map(m => ({ label: m.MedicineOrderPaymentTypeName, value: m.MedicineOrderPaymentTypeID })),
    };
